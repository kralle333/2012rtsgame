package gameclasses.planets
{
	...
	public class Planet extends FlxSprite
	{
		...
		public var gradiantBorder:FlxSprite;
		
		public var shipsToBeAdded:Array = new Array();
		...
		public function Planet(x:int, y:int, size:int, type:String)
		{
			...
			createOverlay();
			productionQueue = new ProductionQueue(this);
		}
		...
		private function checkOwnership():void
		{
			...
			if (firstPlayerShips > secondPlayerShips && firstPlayerShips != 0)
			{
				...
				else if (secondPlayerShips == 0)
				{
					...
					{
						drawOwnership(firstPlayer.color, owner.color, (firstPlayerShips / shipsNeededToOwn) * 225);
					}
				}
				else
				{
			...
			else if (firstPlayerShips < secondPlayerShips)
			{
				...
				else if (firstPlayerShips == 0)
				{
					...
					{
						drawOwnership(owner.color, secondPlayer.color, (secondPlayerShips / shipsNeededToOwn) * 225);
					}
				}
				else
				{
		...
		public function ownsShipOfType(player:Player, type:Class):Boolean
		{
		...
		public function getShipsOfTypeOfPlayer(player:Player, type:Class = null):Array
		{
			
			if (shipsOfPlayers[player] == 0)
			{
				return null;
			}
			...
			for (var i:int = 0; i < ownedShips.length; i++)
			{
				if (player == ownedShips.members[i].owner && ownedShips.members[i].planetFlyingTo == null)
				{
					if (type == null)
					{
						var s:Ship;
						ships.push(ownedShips.members[i]);
					}
					else if (ownedShips.members[i] is type)
					{
						ships.push(ownedShips.members[i]);
					}
				}
